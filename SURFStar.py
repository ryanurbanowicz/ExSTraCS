...
def Run_SURFStar(data,reliefSampleFraction):
    """  Called to run the SURFStar algorithm.  
    ...
    print "Running SURFStar Algorithm..."
    maxInst=int(float(reliefSampleFraction)*len(x)) #param m-number of iteration to run during the ReliefF procedure
    scores=SURFStar(x,y,maxInst,data,reliefSampleFraction)
    print "SURFStar run complete."
    
    return scores


def SURFStar(x,y,maxInst,data,reliefSampleFraction):
    """ Controls major SURFStar loops. """
    ScoreList=[]
    ...
    for inst in range(maxInst): #evaluating each attribute over runIter runs 
        (NN_near,NN_far)=find_data_instances(averageDistance,inst,distanceArray,maxInst) #and finding its nearest neighbours

        for j in range(data.numAttributes):
            if len(NN_near) > 0:
                ScoreList[j]+=evaluate_SURF(x,y,NN_near,j,inst,data,multiclass_map,maxInst)
            if len(NN_far) > 0:
                ScoreList[j]-=evaluate_SURF(x,y,NN_far,j,inst,data,multiclass_map,maxInst)
...
def makeMultiClassMap(y, maxInst, data):
    ...
    for i in range(maxInst):
        if (not multiclass_map.has_key(y[i])):
...
def find_data_instances(averageDistance,inst,distanceArray,maxInst):
    ...
    for j in range(len(min_indices)):
        NN_near.append(min_indices[j])
    for j in range(len(max_indices)):
        NN_far.append(max_indices[j])  
...
def evaluate_SURF(x,y,NN,feature,inst,data,multiclass_map,maxInst):  
    ...
    if data.discretePhenotype:
        if len(data.phenotypeList) > 2: #multiclass endpoint
            ...
            for each in class_Store:
                diff_miss += (class_Store[each][0]/float(missSum))*class_Store[each][1]
